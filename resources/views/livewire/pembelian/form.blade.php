<div class="">
                        ...
                        <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                            ...
                            <td class="px-6 py-4 text-left">
                                <input type="text" wire:model.live="items.{{ $index }}.nama"
                                    placeholder="Nama Barang" @disabled(empty($supplier_id)) @class([
                            ...
                            <td class="px-6 py-4 text-left">
                                <input type="number" wire:model.live="items.{{ $index }}.qty" min="1"
                                    @disabled(empty($supplier_id)) @class([
                                        'bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2',
                                        empty($supplier_id) ? 'cursor-not-allowed opacity-50' : '',
                                    ])>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center flex justify-center items-center gap-3">
                                ...
                                @if (count($items) > 1)
                                    <button type="button" wire:click="removeItem({{ $index }})"
                                        @disabled(empty($supplier_id)) @class([
                                            'bg-danger-100 text-danger-800 text-sm font-medium px-2.5 py-1 rounded-sm',
                                            empty($supplier_id) ? 'cursor-not-allowed opacity-50' : '',
                                        ])>
                                ...
                                @if (
                                    ...
                                    <button type="button" wire:click="addItem" @disabled(empty($supplier_id))
                                        @class([
                                            'bg-primary-100 text-primary-800 text-sm font-medium px-2.5 py-1 rounded-sm',
                                            empty($supplier_id) ? 'cursor-not-allowed opacity-50' : '',
                                        ])>
    ...
    @if (count($items) >= 1 && $isValid)
        <div class="mt-4 flex justify-end">
            <button wire:click="simpan" type="button" @disabled(empty($supplier_id)) @class([
                'text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center shadow-md',
                empty($supplier_id) ? 'cursor-not-allowed opacity-50' : '',
            ])>
                Simpan
            </button>
