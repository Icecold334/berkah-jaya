...
<head>
  <meta charset="UTF-8">
  <title>Nota Penjualan {{ $penjualan->no_struk }}</title>
  <style>
    body {
...
<body>
  ...
  <table class="no-border">
    <tr>
      ...
      <td style="width: 30%; text-align:right; font-size:10px;">
        ...
        <p>Kode: <strong>{{ $penjualan->no_struk }}</strong></p>
      </td>
    </tr>
  </table>

  <p style="margin:4px 0;"><strong>Nota Penjualan</strong></p>

  {{-- CUSTOMER INFO (sementara skip / placeholder) --}}
  <table class="no-border" style="margin-bottom:8px;">
    {{-- <tr>
      <td style="width:20%;"><strong>Customer</strong></td>
      <td style="width:80%;">{{ $penjualan->customer->nama ?? '-' }}</td>
    </tr> --}}
    {{-- <tr>
      <td><strong>Status Pajak</strong></td>
      <td>{{ $penjualan->kena_pajak ? 'Kena Pajak' : 'Non Pajak' }}</td>
    </tr> --}}
  </table>
  ...
  <table>
    ...
    <tbody>
      ...
      @php $rowTotal = $item->subtotal; @endphp
      <tr>
